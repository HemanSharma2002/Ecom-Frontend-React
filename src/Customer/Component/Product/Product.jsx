...
const filters = [
  ...
  },
  {
    ...
    options: [
      ...
      { value: 'textured', label: 'Textured', checked: false },
  ...
  },
  {
    ...
    name: 'Availability',
    options: [
      { value: 'avilable', label: 'Avilable', checked: false },
...
export default function Product({items}) {
  ...
  const handeleFilter=(value,sectionId)=>{
    const searchParams=new URLSearchParams(location.search)
    let filterValue=searchParams.getAll(sectionId)
    // console.log(filterValue[0].split(",").includes(value))
  ...
  function isInParams(value,sectionId){
      const params=new URLSearchParams(location.search)
      // console.log("1")
      // console.log(sectionId)
      let filterValues=params.getAll(sectionId)
      // console.log(filterValues)
      if(filterValues.length>0){
        filterValues[0].split(",").filter((items)=>{
          if(items===value){
            return true
          }
          else
      ...
      <div>
                                ...
                                {section.options.map((option, optionIdx) => (
                                  <div key={option.value} className="flex items-center">
                                    <input
                                      ...
                                      onChange={()=>handeleFilter(option.value,section.id)}
                                      name={`${section.id}[]`}
        ...
        <main className="mx-auto w-full px-4 sm:px-6 lg:px-8">
          ...
          <section aria-labelledby="products-heading" className="pb-24 pt-6">
            ...
            <div className="grid grid-cols-1 gap-x-8 gap-y-10 md:grid-cols-4 h-screen  ">
              {/* Filters */}
              <form className="hidden lg:block overflow-scroll px-2">
                <h3 className="sr-only">Categories</h3>
              ...
              {/* Product grid */}
              <div className="md:col-span-3 w-full  overflow-scroll">{
              /* Your content */
              <div className=" flex flex-wrap justify-center w-full h-full overflow-scroll  ">
                    {
                        // mens_kurta.map((item)=><ProductCard item={item}/>)
                    }
              </div>
